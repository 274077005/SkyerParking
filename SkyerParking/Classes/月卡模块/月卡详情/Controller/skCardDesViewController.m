...
- (cardDesViews *)viewCardDes{
    if (nil==_viewCardDes) {
        ...
        @weakify(self)
        [[_viewCardDes.btnPayforMonthCard rac_signalForControlEvents:(UIControlEventTouchUpInside)] subscribeNext:^(__kindof UIControl * _Nullable x) {
            @strongify(self)
            if (self.money>0) {
                skMonthCardPayViewController *view=[[skMonthCardPayViewController alloc] init];
                view.modelOther=self.modelOther;
                view.arrCardList=self.arrCardList;
                view.arrPateList=self.arrPateList;
                view.arrSelect=self.arrSelect;
                view.payMoney=self.money;
                view.indexCardType=self.indexCardType;
...
-(void)changMoney:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ...
        self.indexCardType=indexPath.row;
        self.modelMoney=[self.arrCardList objectAtIndex:row];
        self.money=self.modelMoney.price*self.arrSelect.count;
    }else{
        self.money=self.modelMoney.price*self.arrSelect.count;
    }
    
    self.viewCardDes.labPayCount.text=[NSString stringWithFormat:@"%ld元",self.money];
}
