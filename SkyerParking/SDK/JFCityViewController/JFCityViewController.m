...
@property (nonatomic, strong) JFCityHeaderView *headerViewss;
...
- (void)chooseCityWithName:(NSNotification *)info {
    ...
    if ([[cityDic valueForKey:@"cityName"] isEqualToString:@"全城"]) {
        ...
        [_manager currentCity:[kCurrentCityInfoDefaults objectForKey:@"cityNumber"] currentCityName:^(NSString *name) {
            ...
            __strong typeof(self) strongSelf = weakSelf;
            if (strongSelf) {
                strongSelf.headerViewss.cityName = name;
        ...
        _headerViewss.cityName = cityName;
...
- (JFCityHeaderView *)headerView {
    if (!_headerViewss) {//这部分代码是搜索城市的代码
//        _headerViewss = [[JFCityHeaderView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 80)];
//        _headerViewss.delegate = self;
//        _headerViewss.backgroundColor = [UIColor whiteColor];
//        _headerViewss.buttonTitle = @"选择区县";
//        _headerViewss.cityName = [kCurrentCityInfoDefaults objectForKey:@"currentCity"] ? [kCurrentCityInfoDefaults objectForKey:@"currentCity"] : [kCurrentCityInfoDefaults objectForKey:@"locationCity"];
    }
    return _headerViewss;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    _headerViewss.cityName = cell.textLabel.text;
